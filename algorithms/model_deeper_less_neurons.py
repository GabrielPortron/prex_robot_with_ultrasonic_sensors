...
class QvalueNetwork(nn.Module):
    def __init__(self, state_dim, action_dim, n_hidden_filters=256):
        ...
        self.fc1 = nn.Linear(state_dim[0] + action_dim[0], self.n_hidden_filters)
        init_weight(self.fc1, initializer="xavier uniform")

        self.fc1.bias.data.zero_()
        self.hidden2 = nn.Linear(
            in_features=self.n_hidden_filters,
            out_features=int(self.n_hidden_filters / 2),
        )
        init_weight(self.hidden2, initializer="xavier uniform")
        self.hidden2.bias.data.zero_()
        self.hidden3 = nn.Linear(
            in_features=int(self.n_hidden_filters / 2),
            out_features=int(self.n_hidden_filters / 4),
        )
        init_weight(self.hidden3, initializer="xavier uniform")
        self.hidden3.bias.data.zero_()

        self.hidden4 = nn.Linear(
            in_features=int(self.n_hidden_filters / 4),
            out_features=int(self.n_hidden_filters / 8),
        )
        init_weight(self.hidden4, initializer="xavier uniform")
        self.hidden4.bias.data.zero_()

        self.q_value = nn.Linear(
            in_features=int(self.n_hidden_filters / 8), out_features=1
        )
...
class PolicyNetwork(nn.Module):
    def __init__(self, state_dim, action_dim, action_bounds, n_hidden_filters=256):
        ...
        self.fc1 = nn.Linear(state_dim[0], self.n_hidden_filters)
        init_weight(self.fc1, initializer="xavier uniform")
        self.fc1.bias.data.zero_()
        ...
        self.mu = nn.Linear(
            in_features=int(self.n_hidden_filters / 8), out_features=self.action_dim[0]
        )
    ...
    def forward(self, states):
        x = torch.flatten(states, -1)  # Flatten the tensor
        x = F.elu(self.fc1(x))
        x = F.elu(self.hidden2(x))
        x = F.elu(self.hidden3(x))
        x = F.elu(self.hidden4(x))
    ...
    def get_action(self, states):
        ...
        return (
            (action * self.action_bounds[1]).clamp_(
