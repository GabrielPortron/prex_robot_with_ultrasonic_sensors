...
class SAC:
    ...
    def save(self, path, folder_name, n_episode):
        import os
        ...
        torch.save(
            ...
            os.path.join(file_path, self.env_name + f"_policy_{n_episode}_weights.pth"),
        )

    def load_weights(self, path, folder_name, n_n_episode):
        print("loading model...")
        filepath = op.join(
            path,
            "models",
            folder_name,
            self.env_name + "_policy_" + str(n_n_episode) + "_weights.pth",
        )
